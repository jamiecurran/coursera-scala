...
abstract class TweetSet {
  ...
  def searchMostRetweeted(t: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def searchMostRetweeted(t: Tweet): Tweet = t
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val leftRightSet = right.filterAcc(p, leftSet)
    if(p(elem)) leftRightSet.incl(elem) else leftRightSet
  }

  def mostRetweeted: Tweet = searchMostRetweeted(elem)


  def searchMostRetweeted(t: Tweet): Tweet = {
    if(t.retweets < elem.retweets) left.searchMostRetweeted(right.searchMostRetweeted(elem))
    else left.searchMostRetweeted(right.searchMostRetweeted(t))
  }
  ...
  def addToList(tweetList: TweetList): TweetList = {
    ...
    val newSet = remove(tweet)
    new Cons(tweet, newSet.addToList(Nil))
  }
