...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toCharArray.groupBy(char => char).mapValues(_.size).toList.sortBy(result => result._1)
  ...
  // for((x,y) <- occurrence; count <- 1 to y) yield (x, count)
  //
